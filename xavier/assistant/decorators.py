...
def xavier_cmd(add_cmd, is_args=False):
    def cmd(func):
        xavier = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            xavier = bot.tgbot
            perms = await xavier.get_permissions(event.chat_id, event.sender_id)
            user = event.sender_id
            FocusFusi = bot.uid
            if perms.is_admin:
                await func(event)
            if event.sender_id == FocusFusi:
                pass
            elif not user:
                pass
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            xavier = bot.tgbot
            boat = await xavier.get_me()
            perms = await xavier.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
